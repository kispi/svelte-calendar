...
db.transaction(() => {
    ...
    const insertUser = db.prepare('INSERT INTO user (id, name, email, email_verified, image) VALUES (?, ?, ?, ?, ?)');
    ...
    for (const a of oldAccounts) {
        insertAccount.run(
            ...
            a.refresh_token, a.access_token, a.expires_at,
            a.token_type, a.scope, a.id_token, a.session_state
        );
    }
    ...
    const insertSession = db.prepare('INSERT INTO session (session_token, user_id, expires) VALUES (?, ?, ?)');
    ...
    const vtExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='verificationToken'").get();
    if (vtExists) {
        db.prepare('ALTER TABLE verificationToken RENAME TO verification_token_old').run();
        db.prepare(`
        ...
        const insertVT = db.prepare('INSERT INTO verification_token (identifier, token, expires) VALUES (?, ?, ?)');
